...
- **Eliminacja Stutteringu i Rozbłysków (Shader Recompilation):** Przebudowano warunkowe renderowanie wszystkich świateł w `Scene3D` oraz `CaseGeometry` ze zmiennych logicznych na sztywną manipulację parametrem `intensity={0}`. Eliminuje to potężny spadek wydajności (przeładowanie shaderów przy zmianie liczby świateł) oraz tzw. efekt "Białego Flasha" sprzężonego z Bloomem przy naciskaniu włączników w Menu.
