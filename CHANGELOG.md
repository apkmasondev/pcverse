...
- **Wyeliminowanie Błysku (Soft-Start dla Emissive):** Wykryto, że materiały ze stałą początkową wartością `emissiveIntensity` (np. 1.5 - 3.0) w połączeniu z flagą `toneMapped: false` przygniatały bufor renderujący w pierwszej klatce. Zaimplementowano płynne "rozpalanie" od 0 (przy użyciu `useFrame` i `THREE.MathUtils.lerp`) w każdej z geometrii 3D (`GPU`, `PSU`, `RAM`, `Motherboard`, `Case`, `CPUCooler`), a także zwiększono rygor dla wyzwalania `Bloom` poprzez podniesienie `luminanceThreshold` do 1.0. Powoduje to ostateczne i trwałe zażegnanie problemu rozbłysków na starcie aplikacji.
