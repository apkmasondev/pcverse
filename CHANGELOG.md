...
- **Odblokowanie Światła Pokoju (RectAreaLight)**: Przywrócono fotorealistyczne oświetlenie krawędziowe pokoju (`RectAreaLight`) dla użytkowników korzystających z dedykowanych kart graficznych (Desktop), wyłączając je dyskretnie wyłącznie na słabszych zintegrowanych układach mobilnych (APU), zachowując idealny balans między jakością obrazu a FPS.
