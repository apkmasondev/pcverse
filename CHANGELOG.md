...
- **Podświetlanie Etykiet w Budowie**: Etykieta komponentu, który aktualnie ma zostać zamontowany w Trybie Budowy, otrzymuje teraz takie same akcenty wizualne (ramka, blask, tło) jak przy najechaniu na nią kursorem. Efekt jest całkowicie wyłączony, gdy użytkownik wyłączy widoczność etykiet.
- **Tekstury RAM na Biurku**: Naprawiono błąd mapowania, przez który tekstura ze złotymi pinami zniknęła na bok kości RAM leżących na biurku (w Trybie Scenografii). Użytkownik doprecyzował również pozycje kości, aby lepiej komponowały się z resztą elementów na stole.
