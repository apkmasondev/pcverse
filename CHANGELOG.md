...
## Etap 53 - Spłata Długu Technologicznego 🧹

- **Martwe światła RGB obudowy**: `groupRef` w `CaseGeometry` nie był podpięty do żadnej grupy, więc `groupRef.current` zawsze pozostawał `null`, a pętla `useFrame` animująca natężenie nie wykonywała się ani razu. Oba wewnętrzne światła punktowe (indygo `#6366f1` i różowe `#ec4899`) trwale tkwiły na `intensity: 0` — funkcja była w kodzie, ale nigdy nie działała. Referencja wskazuje teraz dokładnie grupę `case-internal-lights`. Zweryfikowano pomiarowo: po chwilowym podbiciu natężeń wnętrze obudowy zalewa światło obu barw.
- **Usunięty pośrednik przez `userData`**: Docelowe natężenia były zapisywane w `light.userData.targetIntensity` z poziomu inline'owych callback refów, odtwarzanych przy każdym renderze. Zastąpiono je stałą `RGB_LIGHT_INTENSITIES` odczytywaną wprost w pętli klatki.
- **Callback ref zwracający wartość**: `ref={(el) => tubeRefs.current[i] = el}` w `CableGeometry` zwracał przypisywaną geometrię. React 19 traktuje wartość zwróconą z callback refa jako funkcję czyszczącą i próbuje ją wywołać przy odmontowaniu — to jedyne wystąpienie tego wzorca w repozytorium. Zmieniono wyrażenie na blok.
- **Scena renderowała się w tle Matrixa**: Podczas easter egga drzewo 3D jest ukrywane przez `display: none`, a komentarz w `App.tsx` twierdził, że to „wycisza koszty”. W rzeczywistości pętla renderowania działała dalej. `Scene3D` przełącza teraz `frameloop` na `never`, gdy aktywny jest Matrix. Zmierzone: **46 560 wywołań rysowania WebGL na sekundę → 0**, z poprawnym wznowieniem po wyjściu.
- **Martwa strefa w deszczu Matrixa**: `handleResize` aktualizował wymiary bufora, ale nie przeliczał liczby kolumn ani tablic `drops`/`characters`. Po poszerzeniu okna dołożony obszar zostawał trwale pusty. Zmiana rozmiaru zachowuje stan istniejących kolumn i inicjalizuje nowe, a bufor uwzględnia teraz `devicePixelRatio` (do 2×), więc znaki nie są rozmyte na ekranach o wysokiej gęstości.
- **Dosłowny napis „undefined” w deszczu znaków**: Okresowe czyszczenie bufora (`frameCount % 100`) mogło usunąć poprzedni znak kolumny, po czym `ctx.fillText(prevText, …)` otrzymywał `undefined` i rysował ten łańcuch na ekranie. Dodano osłonę.
- **Animacja Matrixa nie restartuje się przy zmianie stanu**: Efekt miał `isWakingUp` w tablicy zależności, przez co całe `requestAnimationFrame` startowało od nowa, gubiąc stan deszczu. Stan wybudzania czytany jest teraz przez referencję. Timeouty wybudzania i resetu błędu są sprzątane przy odmontowaniu.
- **Uwaga**: Easter egg celowo wymaga odgadnięcia komendy — nie dodano alternatywnego wyjścia klawiszem, żeby nie zdradzać zagadki.
- **Martwy CSS w podsumowaniu Trybu Budowy**: Nakładka „shimmer” miała `absolute inset-0` bez pozycjonowanego rodzica (pozycjonowała się więc względem całego overlaya) oraz warianty `group-hover:` bez przodka z klasą `group`. Przycisk dostał `group relative overflow-hidden`, a efekt działa teraz jako przesuwający się połysk.
- **Deduplikacja paneli eksperta**: `BuildModeExpertPanels` zawierał osiem niemal identycznych, ~40-liniowych bloków JSX różniących się kolorem, ikoną, stroną i szerokością. Wydzielono współdzielony komponent `ExpertPanel`, wspólną listę pasków `StatBars` oraz moduł `expertPanelStyles.ts` z wariantami kolorów i szerokości (pełne, statyczne łańcuchy klas — skaner Tailwind nie widzi nazw sklejanych w runtime). Plik skrócił się z 315 do 194 linii bez zmiany wyglądu; zweryfikowano pozycje i rozwijanie wszystkich ośmiu paneli.
- **Ukryty stan poza magazynem**: `toggleExploded` trzymał flagę `isAnimating` i uchwyt timera w zmiennych modułowych, a obie gałęzie warunku różniły się wyłącznie wartością docelową. Blokada wynika teraz wprost ze stanu (`explodeStep === 1` oznacza trwającą animację), a duplikat gałęzi zniknął. Zweryfikowano sekwencję 0→1→2→1→0 wraz z ignorowaniem kliknięć w trakcie animacji.
- **Nakładające się cykle ekranu ładowania**: `triggerLoading` uruchamiał zagnieżdżone `setTimeout` bez czyszczenia poprzednich, więc dwa szybkie wywołania powodowały, że timer pierwszego chował ekran w trakcie drugiego. Cykl jest teraz resetowany, a progi czasowe wyniesiono do nazwanych stałych.
- **Weryfikacja spójności danych**: Potwierdzono 14 unikalnych identyfikatorów komponentów, pokrycie wszystkich kroków budowy 1–9, obecność wszystkich 39 plików zdjęć przywoływanych w danych oraz brak osieroconych obrazów w `public/images/components`. Przejrzano też dyscyplinę `dispose()` we wszystkich geometriach — każdy `useMemo` tworzący materiał, teksturę lub geometrię ma sparowany cleanup.

## Etap 52 - Audyt Wydajności i Adaptacyjna Jakość Grafiki ⚙️

- **Pełna detekcja możliwości urządzenia**: Nowy moduł `src/utils/deviceCapabilities.ts` uruchamia jednorazową sondę WebGL (natychmiast zwalnianą przez `WEBGL_lose_context`, by nie zajmować slotu kontekstu głównemu Canvasowi) i rozpoznaje: WebGL 2 / WebGL 1 / brak WebGL, nazwę GPU przez `WEBGL_debug_renderer_info` z fallbackiem na `gl.getParameter(gl.RENDERER)`, renderery programowe, układy mobilne i zintegrowane, `MAX_TEXTURE_SIZE`, `navigator.hardwareConcurrency`, `navigator.deviceMemory`, tryb `saveData` oraz `(pointer: coarse)`. Wynik jest memoizowany i mapowany na jeden z trzech poziomów jakości.
- **Poprawiona heurystyka pamięci**: `deviceMemory` bierze udział wyłącznie w kwalifikacji do poziomu `low` (≤ 4 GB). Specyfikacja kwantyzuje tę wartość i ucina na 8 GB, więc rozważany wcześniej warunek „≤ 8 GB” degradowałby jakość na praktycznie każdym desktopie.
- **Runtime'owy monitor FPS**: Nowy komponent `AdaptiveQuality` mierzy realną liczbę klatek w oknach czasowych — pomija pierwsze 2 s (kompilacja shaderów), próbkuje co 1 s przez pierwsze ~8 s, następnie co 5 s jako zabezpieczenie przed throttlingiem termicznym. Spadek poniżej 20 FPS natychmiast przełącza na poziom `low`, a poniżej 35 FPS w dwóch kolejnych oknach obniża jakość o jeden stopień. Duże `delta` (powrót z zakładki w tle) unieważnia okno pomiarowe. Pomiar jest aktywny wyłącznie przy `frameloop="always"` — w trybie `demand` liczba klatek odzwierciedla aktywność użytkownika, nie wydajność sprzętu.
- **Zastąpienie `PerformanceMonitor`**: Usunięto `PerformanceMonitor` z drei, którego `onIncline` przywracał efekty wyłączone wcześniej przez detekcję sprzętową, powodując migotanie ustawień. Nowy magazyn `useQualityStore` pozwala jakości zmieniać się wyłącznie w dół, ponieważ każda zmiana `dpr` realokuje bufor renderu.
- **Trzy poziomy jakości**: Poziomy `high` / `medium` / `low` sterują `dpr`, `powerPreference`, kompletem efektów post-processingu (SMAA, N8AO, DoF, aberracja chromatyczna, mipmapowany Bloom), liczbą cząsteczek Sparkles i Stars, latarką kursora, scenografią biurka, rozdzielczością odbić podłogi oraz modelem oświetlenia. Pełna tabela znajduje się w `README.md`.
- **Świadomy `powerPreference`**: Kontekst WebGL jest tworzony z `high-performance` na sprzęcie wydajnym i `low-power` na słabym — wcześniej parametr nie był ustawiany wcale, więc na laptopach hybrydowych scena mogła trafić na układ zintegrowany. Dodano też `alpha: false`, eliminując zbędne mieszanie kanału alfa dla nieprzezroczystej sceny.
- **LOD odbić i cząsteczek**: `MeshReflectorMaterial` (osobny render target sceny w każdej klatce) schodzi z 512 na 256, a na poziomie `low` jest zastępowany zwykłym, matowym materiałem podłogi. Liczba instancji cząsteczek airflow jest skalowana mnożnikiem poziomu jakości (100% / 60% / 35%), z zachowaniem minimum dla czytelności efektu.
- **Obsługa utraty kontekstu WebGL**: Zdarzenia `webglcontextlost` / `webglcontextrestored` są przechwytywane i kończą się czytelnym komunikatem `role="alert"` z przyciskiem odświeżenia. Wcześniej utrata kontekstu (realny scenariusz przy presji pamięci na słabych GPU) zostawiała użytkownika z zamrożonym, czarnym ekranem.
- **Poprawny komunikat dla WebGL 1**: Bramka w `index.html` rozróżnia teraz brak WebGL od obsługi wyłącznie WebGL 1. Wcześniej urządzenia z działającym WebGL 1 dostawały mylący komunikat „Brak Obsługi WebGL”, mimo że rzeczywistym powodem jest porzucenie WebGL 1 przez Three.js w r163.
- **Tekstowa alternatywa bez WebGL**: Ekran zastępczy zawiera skrócony przewodnik po podzespołach (CPU, GPU, RAM, płyta główna, nośniki, zasilacz, chłodzenie), dzięki czemu strona zachowuje wartość edukacyjną i pozostaje użyteczna dla czytników ekranu oraz robotów indeksujących.
- **Naprawiony preset „Hol / Korytarz”**: Preset `lobby` był wybieralny w interfejsie, ale nie występował w mapie kolorów tła, kolorach siatki ani na liście jasnych środowisk — dziedziczył wygląd po Świcie i nie przyciszał świateł sceny. Kolory tła przeniesiono do jednej mapy `BACKGROUND_COLORS`, a listę jasnych środowisk do stałej `BRIGHT_ENVIRONMENTS`, eliminując trzy rozjeżdżające się źródła prawdy.
- **`dpr` reagujące na zmianę urządzenia**: Wartość `dpr` była inicjalizowana jednorazowo ze stanu `isMobile`, więc obrót tabletu lub zmiana rozmiaru okna poniżej progu mobilnego nie wpływały na rozdzielczość renderowania. Jest teraz wyliczana przy każdym renderze.
- **Stabilna stała aberracji chromatycznej**: `new Vector2(...)` tworzony w ciele renderu wymuszał rekonstrukcję efektu przy każdym przerysowaniu — przeniesiono go do stałej modułowej.
- **Ujednolicony `prefers-reduced-motion`**: `LocalAirflowParticles` korzystał z `useReducedMotion` z Framer Motion (zwracającego `boolean | null`) zamiast z hooka projektu, co dawało niespójne zachowanie względem reszty sceny.
- **Aktualizacja zależności i podatności**: `npm audit` zgłaszał podatność wysokiego poziomu w `nanoid`. Po naprawie i aktualizacji pakietów w ramach zakresów semver (m.in. `@react-three/fiber` 9.6.1 → 9.7.0, `@react-three/drei` 10.7.7 → 10.7.8, `vite` 8.0.16 → 8.2.1) audyt zgłasza 0 podatności. Przy okazji dostosowano `ChromaticAberration` do zmienionych typów w `@react-three/postprocessing` 3.0.5 — usunięte propsy miały wartości domyślne, więc render nie uległ zmianie.
- **Porządki w repozytorium**: Usunięto nieużywane zasoby `src/assets/mobo_back.webp`, `src/assets/vite.svg` i `public/images/wood_texture.webp` (≈219 kB). W `.gitignore` zawężono niebezpieczną regułę `*.cjs`, która ignorowała dowolny plik konfiguracyjny CommonJS w całym repozytorium, do konkretnego katalogu `scripts/`; dodano też reguły dla `.env`, cache narzędzi i plików systemowych Windows.
- **Weryfikacja przeglądarkowa**: Sprawdzono start sceny, przejścia między wszystkimi trzema poziomami jakości, odrzucanie prób podniesienia jakości, rozłożenie na części, wybór podzespołu z panelem edukacyjnym oraz blokowanie niedostępnych przełączników w interfejsie. Konsola pozostaje czysta poza ostrzeżeniem `THREE.Clock: This module has been deprecated`, które pochodzi z wnętrza `@react-three/fiber`.
...
- **Naprawa mobilnego paska sterowania**: Usunięto połączenie `w-max` i `justify-center`, które centrowało szerszą zawartość poza lewą krawędzią obszaru przewijania. Tryb kompaktowy jest teraz rozpoznawany już w pierwszym renderze, co zapobiega pozostawaniu desktopowego przesunięcia animacji `x: -20` na telefonie. Pasek jest kotwiczony symetrycznie do krawędzi viewportu, rozpoczyna układ od pierwszego przycisku (`justify-start`) i zachowuje poziome przewijanie dotykowe aż do ostatniej opcji.
- **Stabilna kamera detali na mobile**: Wyłączono asymetryczny `setViewOffset` na urządzeniach mobilnych, ponieważ był obliczany względem całego okna mimo że Canvas zajmuje tylko `60vh`, przesuwając komponent poza kadr i zaburzając odczucie orbity. Punkt `lookAt` i ostrości DoF pobierają teraz dokładną pozycję świata bezpośrednio z klikniętego obiektu 3D, zamiast zakładać, że wpisana w danych pozycja jest środkiem bryły. Awaryjne wyliczenie uwzględnia pełną transformację modelu: pozycję komponentu, skalę mobilną `0.7` oraz oba przesunięcia grup nadrzędnych. Transformację modelu przeniesiono do jednego wspólnego poziomu sceny, żeby geometria i kamera nie mogły użyć różnych układów współrzędnych. Zaznaczony komponent nie wykonuje też dodatkowego unoszenia w widoku eksplodowanym, a kamera uwzględnia stałe podniesienie tego trybu. Przywrócono również bezpieczny dystans kamery `8`. Desktop zachowuje boczne kadrowanie, liczone z rzeczywistego rozmiaru Canvas.
- **Czysty lint React/TypeScript**: Usunięto 34 błędy i 14 ostrzeżeń, m.in. niejawne typy `any`, kaskadowe aktualizacje stanu w efektach, niestabilne cleanupy referencji oraz niedeterministyczne generowanie cząsteczek podczas renderowania.
- **Bezpieczeństwo zależności**: Wymuszono bezpieczną wersję PostCSS `8.5.25`, eliminując podatność wysokiego poziomu. Końcowy `npm audit --omit=dev` nie zgłasza podatności.
- **Powtarzalne wdrożenia**: Workflow GitHub Pages korzysta teraz z `npm ci` i uruchamia lint przed buildem, dzięki czemu wdrożenie zostaje zatrzymane przy błędzie jakościowym.
- **Optymalizacja bundla**: Zastąpiono przestarzały ręczny podział chunków natywnym `codeSplitting` Rolldown. Największy fragment JavaScript zmniejszył się z około 1,31 MB do około 373 kB, a build nie zgłasza ostrzeżeń o rozmiarze paczek.
- **Mniejszy artefakt produkcyjny**: Wyłączono nieużywane sourcemapy produkcyjne, zmniejszając artefakt wdrożeniowy z około 32,3 MB do 24,7 MB.
- **PWA i indeksowanie**: Poprawiono względne `start_url`, `scope` i identyfikator aplikacji w manifeście, dodano brakujący `sitemap.xml` oraz zaktualizowano `favicon.svg` na charakterystyczną świecącą indygo kropkę zgodną z brandingiem PCVerse.
- **Weryfikacja przeglądarkowa**: Sprawdzono start sceny WebGL, obsługę modala klawiaturą, brak poziomego overflow strony oraz działanie widoków desktopowego i mobilnego.
...
- **Mobilne Blokady Hover**: Przeprowadzono refaktoryzację w `InfoPanel.tsx` polegającą na wstrzyknięciu responsywnych prefixów Tailwind (`md:hover:` oraz `md:group-hover:`) do wszystkich interaktywnych ikon, zdjęć i przycisków. Usunięto to tzw. "lepki hover" na urządzeniach dotykowych, zapobiegając "wieszaniu się" efektów optycznych po oderwaniu palca od ekranu.
