...
const ComponentMesh = memo(({ data, isMobile }: { data: PCComponent, isMobile: boolean }) => {
  ...
  useFrame((_state, delta) => {
    ...
    const angleDist = groupRef.current.quaternion.angleTo(targetQuaternion);
    if (angleDist > 0.001) {
      groupRef.current.quaternion.slerp(targetQuaternion, dt * 5);
      needsInvalidate = true;
    } else if (angleDist > 0) {
  ...
  return (
    <group>
      {isCurrentStep && (
        <mesh ref={ringRef} material={ghostMaterial} rotation={[-Math.PI / 2, 0, 0]}>
