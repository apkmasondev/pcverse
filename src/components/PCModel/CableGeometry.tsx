...
export const CableGeometry = () => {
  ...
  const curve1 = useMemo(() => new CubicBezierCurve3(
    new Vector3(-0.5, -1.8, -0.8), // near PSU (updated Z)
    new Vector3(1.7, -1.8, -1.6),  // routed far right to clear the GPU (GPU ends at X=1.5)
    new Vector3(1.7, 0.4, -1.6),   // coming straight up on the right side of the GPU
  ...
  const curve3 = useMemo(() => new CubicBezierCurve3(
    ...
    new Vector3(1.7, -0.7, -1.7),  // route up the right side behind mobo tray
    new Vector3(1.0, -1.2, -1.7)   // plug into bottom right of mobo
  ), []);
  ...
  const curve6 = useMemo(() => new CubicBezierCurve3(
    new Vector3(-0.4, -1.8, -0.8), // near PSU
    new Vector3(0.6, -1.8, 0.3),   // routed forward and right
    new Vector3(0.6, -0.4, 0.3),   // straight up, standing in front of the GPU
    new Vector3(1.15, -0.1, - 0.55)  // plugs directly into the GPU side edge horizontally
  ), []);
