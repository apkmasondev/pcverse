...
export const CableGeometry = () => {
  ...
  const curve1 = useMemo(() => new THREE.CubicBezierCurve3(
    ...
    new THREE.Vector3(0.9, 0.5, -1.7)    // plug into Motherboard right edge
  ), []);
  ...
  const curve2 = useMemo(() => new THREE.CubicBezierCurve3(
    ...
    new THREE.Vector3(0.9, -0.1, - 0.55)  // plugs directly into the GPU side edge horizontally
  ), []);

  // SATA Data Cable (HDD to Mobo)
  const curve3 = useMemo(() => new THREE.CubicBezierCurve3(
    new THREE.Vector3(1.4, -2.23, 0.25),  // plug into HDD front connector (facing glass)
    new THREE.Vector3(1.3, -1.7, 0.6),   // route forward towards glass
    new THREE.Vector3(1.7, -0.7, -1.7),  // route up the right side behind mobo tray
    new THREE.Vector3(1.0, -1.2, -1.7)   // plug into bottom right of mobo
  ), []);

  // SATA Power Cable (HDD to PSU)
  const curve4 = useMemo(() => new THREE.CubicBezierCurve3(
    new THREE.Vector3(1.1, -2.23, 0.25),  // plug into HDD front power connector
    new THREE.Vector3(1.1, -2.18, 0.8),   // route forward towards glass
    new THREE.Vector3(-0.5, -2.18, -0.4), // route along bottom back to PSU
    new THREE.Vector3(-0.5, -1.9, -0.8)  // plug into PSU
  ), []);
  ...
  const curve5 = useMemo(() => new THREE.CatmullRomCurve3([
    ...
    new THREE.Vector3(-0.35, -2.2, -1.85), // Push back behind tray/mobo
  ...
  const curve6 = useMemo(() => new THREE.CubicBezierCurve3(
    new THREE.Vector3(-0.4, -1.8, -0.8), // near PSU
    new THREE.Vector3(0.6, -1.8, 0.3),   // routed forward and right
    new THREE.Vector3(0.6, -0.4, 0.3),   // straight up, standing in front of the GPU
    new THREE.Vector3(1.15, -0.1, - 0.55)  // plugs directly into the GPU side edge horizontally
  ), []);
